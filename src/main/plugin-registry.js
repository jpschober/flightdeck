 ...
 *
 * @param {object[]} plugins            the registered plugins, in the order that
 *                                      decides a tie
 * @param {object}   ctx                whatever the plugins detect on - the
 *                                      terminal observation or a file provider
 * @param {object}   [opts]
 * @param {string[]} [opts.extraKeys]   further keys to carry over from the
 *                                      detection result; missing ones become an
 *                                      empty array
 * @param {function} [opts.onError]     `(plugin, err)` for a plugin that threw;
 *                                      the run continues afterwards
 * @returns {Promise<object[]>}         `{ plugin, confidence, evidence, ...extraKeys }`
 */
async function detectAll(plugins, ctx, opts = {}) {
  const { extraKeys = [], onError = null } = opts;
  const found = [];
  for (const plugin of plugins) {
    ...
    try {
      ...
      if (onError) onError(plugin, e);
      d = null;
    }
    if (d && d.confidence > 0) {
      const entry = { plugin, confidence: d.confidence, evidence: d.evidence || [] };
      for (const key of extraKeys) entry[key] = d[key] || [];
      found.push(entry);
    }
  }
